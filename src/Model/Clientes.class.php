			...
			if( is_bool( array_search('', $_POST) ) ) {
				...
				if( $total_caracteres == 14 || $total_caracteres == 18  ) {
					...
					if( $total_caracteres == 14 ) {
						...
						if( !preg_match('/^\d{2}\.\d{3}\.\d{3}\/\d{4}\-\d{2}$/', $_POST['cpf_cnpj'] ) ) {
				...
				# substitua o false pelo teste lógico correto
				if( !preg_match('/^[a-z0-9.]+@[a-z0-9]+\.[a-z]+\.([a-z]+)?$/i', $_POST['email'] ) ) {
